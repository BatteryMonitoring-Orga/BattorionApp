...
public class EssentialToolsDownloader {
    ...
    public static void Downloader(){
        ...
        for (String[] repo : REPOSITORIES) {
            ...
            try {
                downloadFile(repoZipUrl, zipPath);
                unzip(zipPath, MAIN_FOLDER_PATH);
                Files.deleteIfExists(Paths.get(zipPath));
            } catch (Exception e) {
                JOptionPane.showMessageDialog(null, "❌ Error downloading an essential tools!" + repoName + ": " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
