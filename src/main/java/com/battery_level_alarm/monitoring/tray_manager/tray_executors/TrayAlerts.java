...
public class TrayAlerts {
	public static void showAboutDialog() {
		...
		Platform.runLater(() -> {
			...
			description.setStyle("-fx-background-color: #f9f9f9; -fx-border-color: #ddd; -fx-border-radius: 4px; -fx-background-radius: 4px;");
	...
	private static @NotNull Label getDescription() {
		Label description = new Label("""
			Battorion Tray is a lightweight background utility that keeps you
			informed about your battery status directly from the system tray.
		\t
			It operates silently without opening any main window and provides
			quick access to essential functions like:
	...
	private static void createInstructionAlert(String title, String header, VBox content, String fallbackText) {
		...
		pane.getStylesheets().add(Objects.requireNonNull(TrayAlerts.class.getResource("/com/battery_level_alarm/monitoring/Tray/dialog-style.css")).toExternalForm());
		Stage stage = (Stage) alert.getDialogPane().getScene().getWindow();
		stage.setResizable(false);
		stage.getIcons().add(new Image(Objects.requireNonNull(
				TrayAlerts.class.getResource("/com/battery_level_alarm/monitoring/Tray/info.png")).toExternalForm()));
		alert.showAndWait();
	}
	
	private static VBox getTrayInstructionsVBox(String os) {
		if (os.contains("win")) {
			URL arrowUrl = TrayAlerts.class.getResource("/com/battery_level_alarm/monitoring/Tray/up_arrow.png");
